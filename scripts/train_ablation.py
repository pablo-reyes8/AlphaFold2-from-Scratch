"""Train a named ablation preset on top of a base experiment config.

This launcher resolves one of the repository's prebuilt ablation presets,
prints the resulting metadata when requested, and otherwise runs the same
single-device training stack used by the standard training CLI.
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a prebuilt AlphaFold2 ablation on a single device.")
