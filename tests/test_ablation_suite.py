...
def _make_alphafold2_stub(*, recycle_pair_enabled=True, structure_pair_context_enabled=True):
    ...
    model.recycle_pair_norm = nn.LayerNorm(4)
    return model


def _make_model_parallel_stub(*, recycle_pair_enabled=True, structure_pair_context_enabled=True):
    ...
    wrapper.recycle_pair_norm = nn.LayerNorm(4)
    return wrapper
...
def test_model_parallel_helper_flags_match_plain_model_behavior():
    ...
    wrapper = _make_model_parallel_stub(recycle_pair_enabled=False, structure_pair_context_enabled=False)
...
def test_loss_gracefully_skips_disabled_auxiliary_terms():
    criterion = AlphaFoldLoss(w_fape=0.0, w_dist=0.0, w_plddt=0.0, w_torsion=0.0)
